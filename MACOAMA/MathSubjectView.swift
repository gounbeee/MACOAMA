...
class ImageSaver : NSObject {
    ...
    // 保存の確認とユーザへの告知をしないと、うまく保存できなかった時、何もわからなくなる。
    ...
    var body: some View {
        ...
        if self.parsedJson != nil {
            ...
            Button("EXPORT TO PNG") {
                ...
                let renderer = ImageRenderer(content: self.parsedView)
                            ...
                            ForEach(page.textElems, id: \.self) { elem in
                                Group {
                                    Text(elem.content)
                                        ...
                                        .offset(x: elem.position.x * screenSize.width,
                                                y: elem.position.y * screenSize.height)
